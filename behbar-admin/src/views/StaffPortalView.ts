...
function renderCard(order: OrderRecord): string {
  ...
  const originNavUrl =
    order.originLat != null && order.originLng != null
      ? `https://www.google.com/maps/dir/?api=1&destination=${order.originLat},${order.originLng}`
      : null;
  const destinationNavUrl =
  ...
  return `
    <div class="portal-card" data-portal-card-id="${order.id}">
      ...
      ${order.originNotes ? `<p class="pipeline-note"><strong>یادداشت مبدأ:</strong> ${escapeHtml(order.originNotes)}</p>` : ''}
      ${order.destinationNotes ? `<p class="pipeline-note"><strong>یادداشت مقصد:</strong> ${escapeHtml(order.destinationNotes)}</p>` : ''}
      ...
      <div class="portal-actions">
        ${originNavUrl ? `<a class="btn btn-secondary btn-sm" href="${originNavUrl}" target="_blank" rel="noopener">مسیریابی به مبدأ</a>` : ''}
        ${destinationNavUrl ? `<a class="btn btn-secondary btn-sm" href="${destinationNavUrl}" target="_blank" rel="noopener">مسیریابی به مقصد</a>` : ''}
