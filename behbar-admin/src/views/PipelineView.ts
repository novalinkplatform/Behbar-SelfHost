...
function renderAssignOptions(current: number | null, assignable: StaffRecord[]): string {
  const options = assignable
    ...
    .join('');
  return `<option value="">اختصاص‌نیافته</option>${options}`;
}
  ...
  return `
    <div class="pipeline-card" data-card-id="${order.id}" style="--card-accent:${STATUS_COLORS[order.status] ?? 'var(--muted)'}">
      ...
      <div class="pipeline-row">
        ...
        <span>${order.originCity}${order.originCountry && order.originCountry !== 'ایران' ? ` (${order.originCountry})` : ''}</span>
        <span aria-hidden="true">←</span>
        <span class="icon">${icons.flag}</span>
        <span>${order.destinationCity}${order.destinationCountry && order.destinationCountry !== 'ایران' ? ` (${order.destinationCountry})` : ''}</span>
      </div>
