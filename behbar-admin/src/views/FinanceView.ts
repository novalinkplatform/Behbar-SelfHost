...
const STORAGE_CUSTOM_DOCS_KEY = 'behbar_custom_finance_docs';

function getStoredCustomDocs(): CustomFinanceDoc[] {
  try {
    const raw = localStorage.getItem(STORAGE_CUSTOM_DOCS_KEY);
...
export function initFinanceView(): void {
  ...
  function getUnifiedDocs(): Array<{
    ...
    for (const ord of allOrders) {
      ...
      list.push({
        id: ord.id,
        isCustom: false,
        trackingCode: ord.trackingCode || `BB-${ord.id}`,
        customerName: ord.customerName || 'مشتری سامانه',
        phone: ord.phone || '—',
        serviceLabel: ord.serviceLabel || 'حمل بار',
  ...
  function openAddDocModal(): void {
    ...
    form?.addEventListener('submit', (e) => {
      ...
      const newDoc: CustomFinanceDoc = {
        id: `custom_${Date.now()}`,
        trackingCode: `BB-FIN-${randomSuffix}`,
  ...
  function exportToExcel(): void {
    ...
    link.download = `behbar-financial-report-${dateStamp}.csv`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);

    showToast('فایل اکسل گزارش مالی بهبار با موفقیت ایجاد و دانلود شد.', 'success');
  }
