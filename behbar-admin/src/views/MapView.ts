...
import { fetchRequests, fetchStaff, assignRequest } from '../utils/api.ts';
  ...
  function popupHtml(order: OrderRecord, assignable: StaffRecord[]): string {
    ...
    return `
      <div class="admin-map-popup">
        ...
        <div class="admin-map-popup-row">${order.originCity}${order.originCountry && order.originCountry !== 'ایران' ? ` (${order.originCountry})` : ''} ← ${order.destinationCity}${order.destinationCountry && order.destinationCountry !== 'ایران' ? ` (${order.destinationCountry})` : ''}</div>
