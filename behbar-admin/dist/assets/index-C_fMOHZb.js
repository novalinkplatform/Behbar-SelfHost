...
`).map(e=>e.trim()).filter(Boolean),n=t.filter(e=>!e.startsWith(`- `)),r=t.filter(e=>e.startsWith(`- `)).map(e=>e.slice(2).trim());return{paragraphs:n,list:r.length?r:void 0}}function Mi(e){let t=`https://behbarapp.ir/p/${encodeURIComponent(e.slug)}`;return`
  ...
  `}function ea(e,t){let n=document.querySelector(`.setup-wizard-card`),r=document.getElementById(`setup-progress-fill`),i=document.getElementById(`setup-progress-text`),a=document.getElementById(`setup-back`),o=document.getElementById(`setup-next`),s=document.getElementById(`setup-skip-all`),c=document.getElementById(`setup-general-error`),l=document.getElementById(`setup-vehicle-list`);if(!n||!r||!i||!a||!o||!s||!c||!l)return;let u=0,d={},f=Xi;function p(){l.innerHTML=f.map((e,t)=>`
    ...
    <div class="testimonial-row" data-audit-edit="${e.id}" role="button" tabindex="0">
      ...
      <div class="testimonial-row-body">
        <div class="testimonial-row-head">
          <strong>${ta(e.title)}</strong>
          <span class="article-status-badge article-status-draft">فاقد ${t.join(` و `)}</span>
        </div>
        <div class="job-application-meta"><span dir="ltr">/magazine/${ta(e.slug)}</span></div>
      </div>
    </div>
  `}function ra(){return`
    <div class="view-header">
  ...
  `}function da(e){let t=e.direction===`credit`?`+`:`−`,n=e.direction===`credit`?`var(--success)`:`var(--danger)`;return`
    <div class="activity-log-row">
      <div class="activity-log-main">
  ...
  `}function fa(e){return`
    <div class="editor-sidebar-card">
      <div class="plugin-card-head">
        <h3>${sa(e.staffName)} — ${J(e.amount)}</h3>
      </div>
  ...
  `}function ma(){document.querySelectorAll(`[data-payroll-tab]`).forEach(e=>{e.addEventListener(`click`,()=>{document.querySelectorAll(`[data-payroll-tab]`).forEach(e=>e.classList.remove(`is-active`)),e.classList.add(`is-active`),document.querySelectorAll(`[data-payroll-panel]`).forEach(t=>{t.hidden=t.dataset.payrollPanel!==e.dataset.payrollTab})})});let e=document.getElementById(`balances-error`),t=document.getElementById(`balances-table-body`);async function n(){e.hidden=!0;try{let e=await bt();t.innerHTML=e.length?e.map(ua).join(``):`<tr><td colspan="4">کارمندی ثبت نشده.</td></tr>`}catch(t){e.hidden=!1,e.textContent=t instanceof Error?t.message:`خطایی پیش آمد.`}}let r=document.getElementById(`wallet-tx-modal`),i=document.getElementById(`wallet-tx-modal-title`),a=document.getElementById(`wallet-tx-modal-list`);document.getElementById(`wallet-tx-modal-close`)?.addEventListener(`click`,()=>r.hidden=!0),t.addEventListener(`click`,e=>{let t=e.target.closest(`[data-wallet-view-tx]`);if(t){let e=Number(t.dataset.walletViewTx),n=t.dataset.walletStaffName??``;i.textContent=`تراکنش‌های ${n}`,a.innerHTML=`<p class="settings-panel-hint">در حال بارگذاری...</p>`,r.hidden=!1,xt(e,50,0).then(({transactions:e})=>{a.innerHTML=e.length?e.map(da).join(``):`<p class="settings-panel-hint">تراکنشی ثبت نشده.</p>`});return}let n=e.target.closest(`[data-wallet-adjust]`);n&&(o.dataset.staffId=n.dataset.walletAdjust,o.hidden=!1)});let o=document.getElementById(`wallet-adjustment-modal`);document.getElementById(`wallet-adjustment-modal-close`)?.addEventListener(`click`,()=>o.hidden=!0),document.getElementById(`wallet-adjustment-form`)?.addEventListener(`submit`,e=>{e.preventDefault();let t=Number(o.dataset.staffId),r=document.getElementById(`adjustment-direction`).value,i=Number(document.getElementById(`adjustment-amount`).value),a=document.getElementById(`adjustment-description`).value.trim(),s=document.getElementById(`adjustment-error`),c=document.getElementById(`adjustment-submit`);if(s.hidden=!0,!Number.isFinite(i)||i<=0||!a){s.hidden=!1,s.textContent=`مبلغ و توضیح را کامل وارد کنید.`;return}c.disabled=!0,St(t,r,i,a).then(()=>(o.hidden=!0,document.getElementById(`wallet-adjustment-form`).reset(),n())).catch(e=>{s.hidden=!1,s.textContent=e instanceof Error?e.message:`خطایی پیش آمد.`}).finally(()=>{c.disabled=!1})});let s=document.getElementById(`payroll-error`),c=document.getElementById(`payroll-preview-wrap`),l=document.getElementById(`payroll-preview-body`),u=document.getElementById(`payroll-result`),d=document.getElementById(`payroll-month`);d.value=new Date().toISOString().slice(0,7);function f(e){return`
    ...
    `}document.getElementById(`payroll-preview-btn`)?.addEventListener(`click`,()=>{s.hidden=!0,u.hidden=!0,Dt(d.value).then(e=>{l.innerHTML=e.map(f).join(``),c.hidden=!1}).catch(e=>{s.hidden=!1,s.textContent=e instanceof Error?e.message:`خطایی پیش آمد.`})}),document.getElementById(`payroll-confirm-btn`)?.addEventListener(`click`,()=>{let e=[];document.querySelectorAll(`[data-payroll-entry]`).forEach(t=>{let n=t.querySelector(`[data-field="include"]`),r=t.querySelector(`[data-field="amount"]`);n?.checked&&r&&Number(r.value)>0&&e.push({staffId:Number(t.dataset.payrollEntry),amount:Number(r.value)})}),e.length&&(s.hidden=!0,Ot(d.value,e).then(e=>{u.hidden=!1,u.textContent=`${e.processed} نفر پردازش شد.${e.skipped.length?` (${e.skipped.length} مورد رد شد — قبلاً واریز شده یا نامعتبر بود)`:``}`,n()}).catch(e=>{s.hidden=!1,s.textContent=e instanceof Error?e.message:`خطایی پیش آمد.`}))});let p=document.getElementById(`payouts-error`),m=document.getElementById(`payouts-list`);async function h(){p.hidden=!0;try{let e=await wt(`pending`);m.innerHTML=e.length?e.map(fa).join(``):`<p class="settings-panel-hint">درخواست در انتظاری نیست.</p>`}catch(e){p.hidden=!1,p.textContent=e instanceof Error?e.message:`خطایی پیش آمد.`}}m.addEventListener(`click`,e=>{let t=e.target.closest(`[data-payout-approve]`);if(t){Tt(Number(t.dataset.payoutApprove)).then(()=>Promise.all([h(),n()])).catch(e=>{p.hidden=!1,p.textContent=e instanceof Error?e.message:`خطایی پیش آمد.`});return}let r=e.target.closest(`[data-payout-reject]`);if(r){let e=window.prompt(`دلیل رد (اختیاری):`)??void 0;Et(Number(r.dataset.payoutReject),e).then(()=>h()).catch(e=>{p.hidden=!1,p.textContent=e instanceof Error?e.message:`خطایی پیش آمد.`})}});let g=document.getElementById(`rates-error`),_=document.getElementById(`rates-saved-note`),v=document.getElementById(`role-rates-body`);async function y(){g.hidden=!0;try{let{roles:e}=await re();v.innerHTML=e.map(pa).join(``)}catch(e){g.hidden=!1,g.textContent=e instanceof Error?e.message:`خطایی پیش آمد.`}}v.addEventListener(`click`,e=>{let t=e.target.closest(`[data-role-rate-save]`);if(!t)return;let n=t.closest(`[data-role-rate-row]`),r=Number(n.dataset.roleRateRow),i=Number(n.querySelector(`[data-field="salary"]`).value),a=n.querySelector(`[data-field="bonusType"]`).value,o=Number(n.querySelector(`[data-field="bonusAmount"]`).value);g.hidden=!0,Ct(r,{defaultSalaryAmount:i,defaultBonusType:a,defaultBonusAmount:o}).then(()=>{_.hidden=!1,window.setTimeout(()=>_.hidden=!0,2500)}).catch(e=>{g.hidden=!1,g.textContent=e instanceof Error?e.message:`خطایی پیش آمد.`})}),n(),h(),y()}function ha(e,t,n){return`
    <button type="button" class="home-tile" data-home-tile="${e}">
      <span class="home-tile-icon">${t}</span>
      <span class="home-tile-label">${n}</span>
    </button>
  `}function ga(e){let t=x(e,`content`)||x(e,`stories`)||x(e,`seo`)||x(e,`settings`),n=x(e,`staff`)||x(e,`recruitment`)||x(e,`roles`)||x(e,`wallet`),r=x(e,`settings`)||x(e,`homepage`),i=x(e,`pipeline`)||x(e,`map`),a=[];return i&&a.push(ha(`requests`,u.columns,`درخواست‌ها`)),x(e,`dashboard`)&&a.push(ha(`dashboard`,u.chart,`داشبورد`)),x(e,`chat`)&&a.push(ha(`chat`,u.chat,`چت پشتیبانی`)),t&&a.push(ha(`content`,u.grid,`مدیریت محتوا`)),n&&a.push(ha(`personnel`,u.users,`مدیریت پرسنل`)),x(e,`staff`)&&a.push(ha(`fleet`,u.truck,`مدیریت ناوگان حمل‌ونقل`)),x(e,`plugins`)&&a.push(ha(`plugins`,u.plugin,`افزونه‌ها`)),r&&a.push(ha(`settings`,u.settings,`تنظیمات سایت`)),`<div class="home-grid">${a.join(``)}</div>`}function _a(e){let t=[];return x(e,`pipeline`)&&t.push(ha(`pipeline`,u.columns,`مراحل درخواست‌ها`)),x(e,`map`)&&t.push(ha(`map`,u.map,`نقشه درخواست‌ها`)),`<div class="home-grid">${t.join(``)}</div>`}function va(e){let t=[];return x(e,`content`)&&t.push(ha(`magazine`,u.article,`مجله`)),x(e,`content`)&&t.push(ha(`testimonials`,u.message,`نظرات مشتریان`)),x(e,`stories`)&&t.push(ha(`stories`,u.story,`استوری‌ها`)),x(e,`seo`)&&t.push(ha(`seo`,u.seo,`مدیریت سئو`)),(x(e,`content`)||x(e,`settings`))&&t.push(ha(`media`,u.image,`مدیریت فایل`)),`<div class="home-grid">${t.join(``)}</div>`}function ya(e){let t=[];return x(e,`staff`)&&t.push(ha(`staff`,u.users,`کارمندان`)),x(e,`recruitment`)&&t.push(ha(`jobApplications`,u.briefcase,`فرصت‌های شغلی`)),x(e,`roles`)&&t.push(ha(`roles`,u.lock,`نقش‌ها و اختیارات`)),(x(e,`staff`)||x(e,`roles`))&&t.push(ha(`activityLog`,u.history,`گزارش فعالیت`)),x(e,`wallet`)&&t.push(ha(`payroll`,u.wallet,`حقوق و دستمزد`)),`<div class="home-grid">${t.join(``)}</div>`}function ba(e,t){e.querySelectorAll(`[data-home-tile]`).forEach(e=>{e.addEventListener(`click`,()=>t(e.dataset.homeTile??``))})}async function xa(){try{let e=(await Te()).site_name?.fa?.trim();if(!e||e===`بهبار`)return;document.querySelectorAll(`#login-brand-text, .admin-logo-title, #staff-portal-brand-text`).forEach(t=>{t.textContent?.includes(`بهبار`)&&(t.textContent=t.textContent.replace(/بهبار/g,e))})}catch{}}function Sa(e){return e===`home`?null:e===`requests`||e===`content`||e===`personnel`||e===`dashboard`?`home`:e===`pipeline`||e===`map`?`requests`:e===`magazine`||e===`testimonials`||e===`stories`||e===`seo`||e===`media`?`content`:e===`staff`||e===`jobApplications`||e===`roles`||e===`activityLog`||e===`payroll`?`personnel`:e===`staffHistory`?`staff`:e===`dashboardOrders`||e===`dashboardVisitors`||e===`dashboardStaff`?`dashboard`:e===`magazine-editor`?`magazine`:e===`page-editor`||e===`pages`?`settings`:`home`}function Ca(){let e=document.querySelector(`#app`);if(!e)throw Error(`#app not found`);return e}function wa(){let e=Ca();e.innerHTML=At(),jt(Ma),xa()}var Ta=`home`,Ea=null;function $(e,t=null){let n=document.getElementById(`view-container`),r=document.getElementById(`admin-back-btn`),i=g();if(!(!n||!i)){if(Ta=e,r&&(r.hidden=e===`home`),v({screen:e,extra:e===`magazine-editor`||e===`page-editor`?t:e===`staffHistory`?Ea:void 0}),document.querySelectorAll(`#admin-topbar-nav .admin-topbar-tab`).forEach(t=>{let n=t.dataset.adminTab,r=n===e||n===`dashboard`&&(e===`dashboardOrders`||e===`dashboardVisitors`||e===`dashboardStaff`)||n===`home`&&![`dashboard`,`dashboardOrders`,`dashboardVisitors`,`dashboardStaff`,`pipeline`,`map`,`fleet`].includes(e);t.classList.toggle(`is-active`,!!r)}),e===`home`)n.innerHTML=ga(i),ba(n,e=>$(e));else if(e===`requests`)n.innerHTML=_a(i),ba(n,e=>$(e));else if(e===`content`)n.innerHTML=va(i),ba(n,e=>$(e));else if(e===`personnel`)n.innerHTML=ya(i),ba(n,e=>$(e));else if(e===`dashboard`)n.innerHTML=vn(),yn(e=>$(e));else if(e===`dashboardOrders`)n.innerHTML=bn();else if(e===`dashboardVisitors`)n.innerHTML=xn();else if(e===`dashboardStaff`)n.innerHTML=Sn();else if(e===`pipeline`)n.innerHTML=dn(),fn();else if(e===`map`)n.innerHTML=Dn(),On();else if(e===`staff`)n.innerHTML=Mn(),Nn(e=>{Ea=e,$(`staffHistory`)});else if(e===`staffHistory`){if(!Ea){$(`staff`);return}n.innerHTML=Fn(Ea),In(Ea)}else if(e===`fleet`)n.innerHTML=er(),tr();else if(e===`magazine`)n.innerHTML=_r(),vr(e=>$(`magazine-editor`,e));else if(e===`magazine-editor`)n.innerHTML=Fr(),Ir(t,()=>$(`magazine`));else if(e===`pages`){$(`settings`);return}else e===`page-editor`?(n.innerHTML=Hr(),Ur(t,()=>$(`settings`))):e===`testimonials`?(n.innerHTML=Jr(),Yr()):e===`chat`?(n.innerHTML=ni(),ri()):e===`stories`?(n.innerHTML=si(),ci()):e===`jobApplications`?(n.innerHTML=mi(),hi()):e===`settings`?(n.innerHTML=Vi(),Hi((e,t)=>$(e,t??null))):e===`plugins`?(n.innerHTML=Gi(),Yi()):e===`seo`?(n.innerHTML=ra(),ia(e=>$(`magazine-editor`,e))):e===`media`?(n.innerHTML=Ln(),Rn()):e===`roles`?(n.innerHTML=Bn(),Vn()):e===`activityLog`?(n.innerHTML=vi(),yi()):e===`accountSecurity`?(n.innerHTML=aa(),oa()):e===`myWallet`?(n.innerHTML=or(),lr()):e===`payroll`?(n.innerHTML=la(),ma()):(n.innerHTML=ga(i),ba(n,e=>$(e)))}}function Da(e,t=!1){let n=Ca();n.innerHTML=Rt(e),document.getElementById(`admin-back-btn`)?.addEventListener(`click`,()=>{let e=Sa(Ta);e&&$(e)}),document.getElementById(`logout-btn`)?.addEventListener(`click`,()=>{j(),_(),wa()}),document.getElementById(`admin-account-btn`)?.addEventListener(`click`,()=>$(`accountSecurity`)),document.getElementById(`admin-wallet-btn`)?.addEventListener(`click`,()=>$(`myWallet`)),document.querySelectorAll(`#admin-topbar-nav .admin-topbar-tab`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.adminTab;t&&$(t)})}),document.getElementById(`admin-logo-btn`)?.addEventListener(`click`,()=>{$(x(e,`dashboard`)?`dashboard`:`home`)}),x(e,`settings`)&&ka(),Oa(),Wn(),xa(),x(e,`ai`)&&Ft();let r=t?y():null;r?.screen===`staffHistory`&&r.extra?(Ea=r.extra,$(`staffHistory`)):r?.screen===`magazine-editor`?$(`magazine-editor`,r.extra??null):r?.screen===`page-editor`?$(`page-editor`,r.extra??null):r?.screen?$(r.screen):$(x(e,`dashboard`)?`dashboard`:`home`)}async function Oa(){try{let e=(await Te()).branding,t=e?.logoUrl;t&&document.querySelectorAll(`.admin-logo-mark`).forEach(e=>e.src=t);let n=e?.faviconUrl||t;n&&document.querySelectorAll(`link[rel="icon"]`).forEach(e=>e.href=n)}catch{}}async function ka(){let e=document.getElementById(`admin-sidebar-version`);if(e)try{let t=await gt();e.textContent=t.updateAvailable?`نسخه ${t.current} · نسخه جدید موجود است`:`نسخه ${t.current}`,e.classList.toggle(`has-update`,t.updateAvailable),e.hidden=!1}catch{}}function Aa(e){let t=Ca();t.innerHTML=pr(e),xa(),hr(()=>{j(),_(),wa()})}function ja(e){let t=Ca();t.innerHTML=$i(),xa(),ea(e,()=>Da(e))}function Ma(e,t=!1){if(!S(e)){Aa(e);return}if(!x(e,`settings`)){Da(e,t);return}Te().then(n=>{Qi(n)?ja(e):Da(e,t)}).catch(()=>Da(e,t))}function Na(){if(T(wa),!b()){wa();return}let e=g();e?Ma(e,!0):wa(),te().then(t=>{let n=h();n&&(m(n,t),(!e||e.role!==t.role)&&Ma(t,!0))}).catch(()=>{})}Na();