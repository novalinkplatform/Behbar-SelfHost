...
`).map(e=>e.trim()).filter(Boolean),n=t.filter(e=>!e.startsWith(`- `)),r=t.filter(e=>e.startsWith(`- `)).map(e=>e.slice(2).trim());return{paragraphs:n,list:r.length?r:void 0}}function Fi(e){let t=`https://behbarapp.ir/p/${encodeURIComponent(e.slug)}`;return`
  ...
  `}function ba(){document.querySelectorAll(`[data-payroll-tab]`).forEach(e=>{e.addEventListener(`click`,()=>{document.querySelectorAll(`[data-payroll-tab]`).forEach(e=>e.classList.remove(`is-active`)),e.classList.add(`is-active`),document.querySelectorAll(`[data-payroll-panel]`).forEach(t=>{t.hidden=t.dataset.payrollPanel!==e.dataset.payrollTab})})});let e=document.getElementById(`balances-error`),t=document.getElementById(`balances-table-body`);async function n(){e.hidden=!0;try{let e=await bt();t.innerHTML=e.length?e.map(ga).join(``):`<tr><td colspan="4">کارمندی ثبت نشده.</td></tr>`}catch(t){e.hidden=!1,e.textContent=t instanceof Error?t.message:`خطایی پیش آمد.`}}let r=document.getElementById(`wallet-tx-modal`),i=document.getElementById(`wallet-tx-modal-title`),a=document.getElementById(`wallet-tx-modal-list`);document.getElementById(`wallet-tx-modal-close`)?.addEventListener(`click`,()=>r.hidden=!0),t.addEventListener(`click`,e=>{let t=e.target.closest(`[data-wallet-view-tx]`);if(t){let e=Number(t.dataset.walletViewTx),n=t.dataset.walletStaffName??``;i.textContent=`تراکنش‌های ${n}`,a.innerHTML=`<p class="settings-panel-hint">در حال بارگذاری...</p>`,r.hidden=!1,xt(e,50,0).then(({transactions:e})=>{a.innerHTML=e.length?e.map(_a).join(``):`<p class="settings-panel-hint">تراکنشی ثبت نشده.</p>`});return}let n=e.target.closest(`[data-wallet-adjust]`);n&&(o.dataset.staffId=n.dataset.walletAdjust,o.hidden=!1)});let o=document.getElementById(`wallet-adjustment-modal`);document.getElementById(`wallet-adjustment-modal-close`)?.addEventListener(`click`,()=>o.hidden=!0),document.getElementById(`wallet-adjustment-form`)?.addEventListener(`submit`,e=>{e.preventDefault();let t=Number(o.dataset.staffId),r=document.getElementById(`adjustment-direction`).value,i=Number(document.getElementById(`adjustment-amount`).value),a=document.getElementById(`adjustment-description`).value.trim(),s=document.getElementById(`adjustment-error`),c=document.getElementById(`adjustment-submit`);if(s.hidden=!0,!Number.isFinite(i)||i<=0||!a){s.hidden=!1,s.textContent=`مبلغ و توضیح را کامل وارد کنید.`;return}c.disabled=!0,St(t,r,i,a).then(()=>(o.hidden=!0,document.getElementById(`wallet-adjustment-form`).reset(),n())).catch(e=>{s.hidden=!1,s.textContent=e instanceof Error?e.message:`خطایی پیش آمد.`}).finally(()=>{c.disabled=!1})});let s=document.getElementById(`payroll-error`),c=document.getElementById(`payroll-preview-wrap`),l=document.getElementById(`payroll-preview-body`),u=document.getElementById(`payroll-result`),d=document.getElementById(`payroll-month`);d.value=new Date().toISOString().slice(0,7);function f(e){return`
    ...
    `}document.getElementById(`payroll-preview-btn`)?.addEventListener(`click`,()=>{s.hidden=!0,u.hidden=!0,Dt(d.value).then(e=>{l.innerHTML=e.map(f).join(``),c.hidden=!1}).catch(e=>{s.hidden=!1,s.textContent=e instanceof Error?e.message:`خطایی پیش آمد.`})}),document.getElementById(`payroll-confirm-btn`)?.addEventListener(`click`,()=>{let e=[];document.querySelectorAll(`[data-payroll-entry]`).forEach(t=>{let n=t.querySelector(`[data-field="include"]`),r=t.querySelector(`[data-field="amount"]`);n?.checked&&r&&Number(r.value)>0&&e.push({staffId:Number(t.dataset.payrollEntry),amount:Number(r.value)})}),e.length&&(s.hidden=!0,Ot(d.value,e).then(e=>{u.hidden=!1,u.textContent=`${e.processed} نفر پردازش شد.${e.skipped.length?` (${e.skipped.length} مورد رد شد — قبلاً واریز شده یا نامعتبر بود)`:``}`,n()}).catch(e=>{s.hidden=!1,s.textContent=e instanceof Error?e.message:`خطایی پیش آمد.`}))});let p=document.getElementById(`payouts-error`),m=document.getElementById(`payouts-list`);async function h(){p.hidden=!0;try{let e=await wt(`pending`);m.innerHTML=e.length?e.map(va).join(``):`<p class="settings-panel-hint">درخواست در انتظاری نیست.</p>`}catch(e){p.hidden=!1,p.textContent=e instanceof Error?e.message:`خطایی پیش آمد.`}}m.addEventListener(`click`,e=>{let t=e.target.closest(`[data-payout-approve]`);if(t){Tt(Number(t.dataset.payoutApprove)).then(()=>Promise.all([h(),n()])).catch(e=>{p.hidden=!1,p.textContent=e instanceof Error?e.message:`خطایی پیش آمد.`});return}let r=e.target.closest(`[data-payout-reject]`);if(r){let e=window.prompt(`دلیل رد (اختیاری):`)??void 0;Et(Number(r.dataset.payoutReject),e).then(()=>h()).catch(e=>{p.hidden=!1,p.textContent=e instanceof Error?e.message:`خطایی پیش آمد.`})}});let g=document.getElementById(`rates-error`),_=document.getElementById(`rates-saved-note`),v=document.getElementById(`role-rates-body`);async function y(){g.hidden=!0;try{let{roles:e}=await re();v.innerHTML=e.map(ya).join(``)}catch(e){g.hidden=!1,g.textContent=e instanceof Error?e.message:`خطایی پیش آمد.`}}v.addEventListener(`click`,e=>{let t=e.target.closest(`[data-role-rate-save]`);if(!t)return;let n=t.closest(`[data-role-rate-row]`),r=Number(n.dataset.roleRateRow),i=Number(n.querySelector(`[data-field="salary"]`).value),a=n.querySelector(`[data-field="bonusType"]`).value,o=Number(n.querySelector(`[data-field="bonusAmount"]`).value);g.hidden=!0,Ct(r,{defaultSalaryAmount:i,defaultBonusType:a,defaultBonusAmount:o}).then(()=>{_.hidden=!1,window.setTimeout(()=>_.hidden=!0,2500)}).catch(e=>{g.hidden=!1,g.textContent=e instanceof Error?e.message:`خطایی پیش آمد.`})}),n(),h(),y()}function xa(e,t,n){return`
    <button type="button" class="home-tile" data-home-tile="${e}">
      <span class="home-tile-icon">${t}</span>
      <span class="home-tile-label">${n}</span>
    </button>
  `}function Sa(e){let t=x(e,`content`)||x(e,`stories`)||x(e,`seo`)||x(e,`settings`),n=x(e,`staff`)||x(e,`recruitment`)||x(e,`roles`)||x(e,`wallet`),r=x(e,`settings`)||x(e,`homepage`),i=x(e,`pipeline`)||x(e,`map`),a=[];return a.push(xa(`dashboard`,u.chart,`داشبورد`)),i&&a.push(xa(`requests`,u.columns,`درخواست‌ها`)),x(e,`staff`)&&a.push(xa(`fleet`,u.truck,`مدیریت ناوگان حمل‌ونقل`)),r&&a.push(xa(`settings`,u.settings,`تنظیمات سایت`)),x(e,`chat`)&&a.push(xa(`chat`,u.chat,`چت پشتیبانی`)),t&&a.push(xa(`content`,u.grid,`مدیریت محتوا`)),n&&a.push(xa(`personnel`,u.users,`مدیریت پرسنل`)),x(e,`plugins`)&&a.push(xa(`plugins`,u.plugin,`افزونه‌ها`)),`<div class="home-grid">${a.join(``)}</div>`}function Ca(e){let t=[];return x(e,`pipeline`)&&t.push(xa(`pipeline`,u.columns,`مراحل درخواست‌ها`)),x(e,`map`)&&t.push(xa(`map`,u.map,`نقشه درخواست‌ها`)),`<div class="home-grid">${t.join(``)}</div>`}function wa(e){let t=[];return x(e,`content`)&&t.push(xa(`magazine`,u.article,`مجله`)),x(e,`content`)&&t.push(xa(`testimonials`,u.message,`نظرات مشتریان`)),x(e,`stories`)&&t.push(xa(`stories`,u.story,`استوری‌ها`)),x(e,`seo`)&&t.push(xa(`seo`,u.seo,`مدیریت سئو`)),(x(e,`content`)||x(e,`settings`))&&t.push(xa(`media`,u.image,`مدیریت فایل`)),`<div class="home-grid">${t.join(``)}</div>`}function Ta(e){let t=[];return x(e,`staff`)&&t.push(xa(`staff`,u.users,`کارمندان`)),x(e,`recruitment`)&&t.push(xa(`jobApplications`,u.briefcase,`فرصت‌های شغلی`)),x(e,`roles`)&&t.push(xa(`roles`,u.lock,`نقش‌ها و اختیارات`)),(x(e,`staff`)||x(e,`roles`))&&t.push(xa(`activityLog`,u.history,`گزارش فعالیت`)),x(e,`wallet`)&&t.push(xa(`payroll`,u.wallet,`حقوق و دستمزد`)),`<div class="home-grid">${t.join(``)}</div>`}function Ea(e,t){e.querySelectorAll(`[data-home-tile]`).forEach(e=>{e.addEventListener(`click`,()=>t(e.dataset.homeTile??``))})}async function Da(){try{let e=(await Te()).site_name?.fa?.trim();if(!e||e===`بهبار`)return;document.querySelectorAll(`#login-brand-text, .admin-logo-title, #staff-portal-brand-text`).forEach(t=>{t.textContent?.includes(`بهبار`)&&(t.textContent=t.textContent.replace(/بهبار/g,e))})}catch{}}function Oa(e){return e===`home`?null:e===`requests`||e===`content`||e===`personnel`||e===`dashboard`||e===`settings`||e===`plugins`||e===`chat`||e===`fleet`?`home`:e===`pipeline`||e===`map`?`requests`:e===`magazine`||e===`testimonials`||e===`stories`||e===`seo`||e===`media`?`content`:e===`staff`||e===`jobApplications`||e===`roles`||e===`activityLog`||e===`payroll`?`personnel`:e===`staffHistory`?`staff`:e===`dashboardOrders`||e===`dashboardVisitors`||e===`dashboardStaff`?`dashboard`:e===`magazine-editor`?`magazine`:e===`page-editor`||e===`pages`?`settings`:`home`}function ka(){let e=document.querySelector(`#app`);if(!e)throw Error(`#app not found`);return e}function Aa(){let e=ka();e.innerHTML=jt(),Mt(Ha),Da()}var ja={home:``,dashboard:`dashboard`,requests:`requests`,pipeline:`pipeline`,map:`map`,fleet:`fleet`,chat:`chat`,content:`content`,personnel:`personnel`,settings:`settings`,plugins:`plugins`,seo:`seo`,media:`media`,staff:`staff`,roles:`roles`,testimonials:`testimonials`,stories:`stories`,jobApplications:`job-applications`,activityLog:`activity-log`,accountSecurity:`account-security`,myWallet:`wallet`,payroll:`payroll`,magazine:`magazine`,"magazine-editor":`magazine-editor`,pages:`settings`,"page-editor":`page-editor`,dashboardOrders:`dashboard/orders`,dashboardVisitors:`dashboard/visitors`,dashboardStaff:`dashboard/staff`,staffHistory:`staff/history`},Ma={"":`home`,dashboard:`dashboard`,requests:`requests`,pipeline:`pipeline`,map:`map`,fleet:`fleet`,chat:`chat`,content:`content`,personnel:`personnel`,settings:`settings`,plugins:`plugins`,seo:`seo`,media:`media`,staff:`staff`,roles:`roles`,testimonials:`testimonials`,stories:`stories`,"job-applications":`jobApplications`,"activity-log":`activityLog`,"account-security":`accountSecurity`,wallet:`myWallet`,payroll:`payroll`,magazine:`magazine`,"magazine-editor":`magazine-editor`,"page-editor":`page-editor`,"dashboard/orders":`dashboardOrders`,"dashboard/visitors":`dashboardVisitors`,"dashboard/staff":`dashboardStaff`,"staff/history":`staffHistory`};function Na(){let e=window.location.pathname.replace(/^\/management\/?/,``).replace(/\/+$/,``);if(!e)return{screen:`home`};if(Ma[e])return{screen:Ma[e]};let t=e.split(`/`);if(t.length===2&&(t[0]===`magazine-editor`||t[0]===`page-editor`)){let e=t[0],n=parseInt(t[1],10);return{screen:e,detailId:isNaN(n)?null:n}}return null}function Pa(e,t=null){let n=ja[e]??``,r=n;(e===`magazine-editor`||e===`page-editor`)&&t&&(r=`${n}/${t}`);let i=r?`/management/${r}`:`/management/`;window.location.pathname!==i&&window.history.pushState({screen:e,editingDetailId:t},``,i)}var Fa=`home`,Ia=null;function $(e,t=null,n=!0){let r=document.getElementById(`view-container`),i=document.getElementById(`admin-back-btn`),a=g();if(!(!r||!a)){if(Fa=e,i&&(i.hidden=e===`home`),v({screen:e,extra:e===`magazine-editor`||e===`page-editor`?t:e===`staffHistory`?Ia:void 0}),n&&Pa(e,t),e===`home`)r.innerHTML=Sa(a),Ea(r,e=>$(e));else if(e===`requests`)r.innerHTML=Ca(a),Ea(r,e=>$(e));else if(e===`content`)r.innerHTML=wa(a),Ea(r,e=>$(e));else if(e===`personnel`)r.innerHTML=Ta(a),Ea(r,e=>$(e));else if(e===`dashboard`)r.innerHTML=yn(),bn(e=>$(e));else if(e===`dashboardOrders`)r.innerHTML=xn();else if(e===`dashboardVisitors`)r.innerHTML=Sn();else if(e===`dashboardStaff`)r.innerHTML=Cn();else if(e===`pipeline`)r.innerHTML=fn(),pn();else if(e===`map`)r.innerHTML=On(),kn();else if(e===`staff`)r.innerHTML=Nn(),Pn(e=>{Ia=e,$(`staffHistory`)});else if(e===`staffHistory`){if(!Ia){$(`staff`);return}r.innerHTML=In(Ia),Ln(Ia)}else if(e===`fleet`)r.innerHTML=tr(),nr();else if(e===`magazine`)r.innerHTML=vr(),yr(e=>$(`magazine-editor`,e));else if(e===`magazine-editor`)r.innerHTML=Ir(),Lr(t,()=>$(`magazine`));else if(e===`pages`){$(`settings`);return}else e===`page-editor`?(r.innerHTML=Ur(),Wr(t,()=>$(`settings`))):e===`testimonials`?(r.innerHTML=Yr(),Xr()):e===`chat`?(r.innerHTML=ri(),ii()):e===`stories`?(r.innerHTML=ci(),li()):e===`jobApplications`?(r.innerHTML=_i(),vi()):e===`settings`?(r.innerHTML=Wi(),Gi((e,t)=>$(e,t??null))):e===`plugins`?(r.innerHTML=Yi(),$i()):e===`seo`?(r.innerHTML=la(),ua(e=>$(`magazine-editor`,e))):e===`media`?(r.innerHTML=Rn(),zn()):e===`roles`?(r.innerHTML=Vn(),Hn()):e===`activityLog`?(r.innerHTML=xi(),Si()):e===`accountSecurity`?(r.innerHTML=da(),fa()):e===`myWallet`?(r.innerHTML=sr(),ur()):e===`payroll`?(r.innerHTML=ha(),ba()):(r.innerHTML=Sa(a),Ea(r,e=>$(e)))}}function La(e,t=!1){let n=ka();n.innerHTML=zt(e),document.getElementById(`admin-back-btn`)?.addEventListener(`click`,()=>{let e=Oa(Fa);e&&$(e)}),document.getElementById(`logout-btn`)?.addEventListener(`click`,()=>{j(),_(),Aa()}),document.getElementById(`admin-account-btn`)?.addEventListener(`click`,()=>$(`accountSecurity`)),document.getElementById(`admin-wallet-btn`)?.addEventListener(`click`,()=>$(`myWallet`));function r(){let e=document.documentElement.getAttribute(`data-theme`)===`dark`,t=document.getElementById(`admin-theme-sun`),n=document.getElementById(`admin-theme-moon`);t&&n&&(t.hidden=!e,n.hidden=e)}r(),document.getElementById(`admin-theme-toggle-btn`)?.addEventListener(`click`,()=>{document.documentElement.getAttribute(`data-theme`)===`dark`?(document.documentElement.removeAttribute(`data-theme`),localStorage.setItem(`behbar_admin_theme`,`light`)):(document.documentElement.setAttribute(`data-theme`,`dark`),localStorage.setItem(`behbar_admin_theme`,`dark`)),r()}),document.getElementById(`admin-logo-btn`)?.addEventListener(`click`,()=>{$(`home`)}),window.addEventListener(`popstate`,e=>{let t=e.state;if(t?.screen)$(t.screen,t.editingDetailId??null,!1);else{let e=Na();e?$(e.screen,e.detailId??null,!1):$(`home`,null,!1)}}),x(e,`settings`)&&za(),Ra(),Gn(),Da(),x(e,`ai`)&&It();let i=Na(),a=t?y():null;i&&i.screen!==`home`?$(i.screen,i.detailId??null,!1):a?.screen===`staffHistory`&&a.extra?(Ia=a.extra,$(`staffHistory`)):a?.screen===`magazine-editor`?$(`magazine-editor`,a.extra??null):a?.screen===`page-editor`?$(`page-editor`,a.extra??null):a?.screen&&a.screen!==`home`?$(a.screen):$(`home`,null,!0)}async function Ra(){try{let e=await Te(),t=e.branding,n=t?.logoUrl;n&&document.querySelectorAll(`.admin-logo-mark`).forEach(e=>e.src=n);let r=t?.faviconUrl||n;r&&document.querySelectorAll(`link[rel="icon"]`).forEach(e=>e.href=r);let i=e.site_name;i?.fa?.trim()&&(document.title=`${i.fa.trim()} - پنل مدیریت`,document.querySelectorAll(`.admin-logo-title`).forEach(e=>e.textContent=i.fa.trim()))}catch{}}async function za(){let e=document.getElementById(`admin-sidebar-version`);if(e)try{let t=await gt();e.textContent=t.updateAvailable?`نسخه ${t.current} · نسخه جدید موجود است`:`نسخه ${t.current}`,e.classList.toggle(`has-update`,t.updateAvailable),e.hidden=!1}catch{}}function Ba(e){let t=ka();t.innerHTML=mr(e),Da(),gr(()=>{j(),_(),Aa()})}function Va(e){let t=ka();t.innerHTML=aa(),Da(),oa(e,()=>La(e))}function Ha(e,t=!1){if(!S(e)){Ba(e);return}if(!x(e,`settings`)){La(e,t);return}Te().then(n=>{ra(n)?Va(e):La(e,t)}).catch(()=>La(e,t))}function Ua(){if(T(Aa),!b()){Aa();return}let e=g();e?Ha(e,!0):Aa(),te().then(t=>{let n=h();n&&(m(n,t),(!e||e.role!==t.role)&&Ha(t,!0))}).catch(()=>{})}Ua();