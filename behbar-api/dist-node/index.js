...
async function publicGetSettings(env, origin) {
    ...
    if (env.RUNTIME === 'selfhost') {
        const siteNameObj = settings.site_name;
...
async function ensureUserEventsTable(db) {
    if (userEventsTableEnsured)
        return;
    try {
        await db.exec(`
...
async function publicTrackEvent(request, env, origin) {
    let body;
    try {
        body = (await request.json());
    }
...
async function adminGetAnalytics(request, env, origin) {
    if (!(await requireStaff(request, env, 'dashboard')))
        return json({ error: 'دسترسی نداری.' }, 401, origin);
    await ensureUserEventsTable(env.DB);
    const [totalViewsRow, uniqueVisitorsRow, todayViewsRow, todayVisitorsRow, activeOnlineRow, ordersCountRow, dailyRows, topPages, topReferrers, deviceRows, behaviorEventsRows,] = await Promise.all([
        ...
        env.DB.prepare("SELECT COUNT(*) as count FROM page_views WHERE created_at >= datetime('now', 'start of day')").first(),
        env.DB.prepare("SELECT COUNT(DISTINCT visitor_id) as count FROM page_views WHERE created_at >= datetime('now', 'start of day')").first(),
        env.DB.prepare("SELECT COUNT(DISTINCT visitor_id) as count FROM page_views WHERE created_at >= datetime('now', '-15 minutes')").first(),
        env.DB.prepare("SELECT COUNT(*) as count FROM requests WHERE created_at >= datetime('now', '-30 days')").first(),
       ...
       FROM page_views WHERE created_at >= datetime('now', '-30 days') GROUP BY referrer ORDER BY count DESC LIMIT 8`).all(),
        ...
        env.DB.prepare(`SELECT event_type, COUNT(*) as count FROM user_events WHERE created_at >= datetime('now', '-30 days') GROUP BY event_type ORDER BY count DESC`).all().catch(() => ({ results: [] })),
    ]);
