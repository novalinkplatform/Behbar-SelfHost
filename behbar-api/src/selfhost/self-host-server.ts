...
import { SqliteD1, bootstrapIfEmpty, applyPendingSelfhostMigrations, ensureDataDir } from './sqlite-d1.ts';
import { LocalMediaBucket } from './local-media.ts';
import { toFetchRequest, sendFetchResponse } from './http-adapter.ts';
import { handleRequest, runDailyScheduledTasks, registerTrialLicenseIfNeeded } from '../index.ts';
...
async function createFirstAdminIfNeeded(): Promise<void> {
