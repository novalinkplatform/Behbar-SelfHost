// در حالت خوداستقرار، هر خریدار روی سرور خودش یک دیتابیس SQLite دارد که فقط یک‌بار (وقتی خالی است)
// از schema.sql بوت‌استرپ می‌شود؛ migration-*.sql کلادفلر هرگز روی آن اجرا نمی‌شوند. برای اینکه بعد از
// هر «آپدیت» (پول ایمیج جدید) ستون‌های جدیدی که به schema.sql اضافه شده به دیتابیس‌های قبلاً نصب‌شده هم
// برسند، همین تغییرات ساختاری این‌جا هم به‌صورت idempotent فهرست می‌شوند و در هر بار بالا آمدن سرور
// (نه فقط دیتابیس خالی) بررسی و در صورت لزوم اجرا می‌شوند — نسخه‌ی اعمال‌شده در PRAGMA user_version نگه داشته می‌شود.
export interface SelfhostMigration {
  version: number;
  sql: string;
}

export const SELFHOST_MIGRATIONS: SelfhostMigration[] = [
  {
    version: 14,
    sql: `
