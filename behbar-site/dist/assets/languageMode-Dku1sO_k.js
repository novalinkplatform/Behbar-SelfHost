import{S as e,_ as t,b as n,f as r,g as i,h as a,m as o,v as s,x as c,y as l}from"./appReady-u0XdWzsQ.js";var u={phoneDisplay:`021-200200`,phoneTelHref:`tel:+9821200200`,socialLinks:[{id:`wa1`,platform:`whatsapp`,label:`واتساپ`,url:`https://wa.me/9821200200`},{id:`tg1`,platform:`telegram`,label:`تلگرام`,url:`#`},{id:`ig1`,platform:`instagram`,label:`اینستاگرام`,url:`#`},{id:`em1`,platform:`mail`,label:`ایمیل`,url:`#`}]};function d(e){return e?{...e,socialLinks:e.socialLinks??u.socialLinks}:u}function f(e){return s(d(e).phoneDisplay)}function p(t){let r=d(t?.contact);return`
  ...
  `}var N=null;function P(e){te();let t=document.querySelector(`.site-header`);if(t){if(e){let n=t.querySelector(`.header-pill`);if(n&&!n.querySelector(`.header-extra-item`)){let t=j(e);if(t.length){let e=t.map(e=>`
              <a class="header-item header-extra-item" href="${e.href}">
                <span class="icon">${e.icon}</span>
                <span class="header-label">${e.label}</span>
              </a>
            `).join(``);n.insertAdjacentHTML(`beforeend`,e)}}}N&&window.removeEventListener(`scroll`,N),N=()=>{t.classList.toggle(`is-scrolled`,window.scrollY>24),document.body.classList.toggle(`page-scrolled`,window.scrollY>200)},N(),window.addEventListener(`scroll`,N,{passive:!0})}}function F(e){let t=[];t.push({href:`/`,label:n(`خانه`,`Home`)});let r=e?.legal_pages;if((!r?.terms||r.terms.showInFooter!==!1)&&t.push({href:`/terms`,label:n(r?.terms?.title,r?.terms?.titleEn)||n(`قوانین و مقررات`,`Terms & Conditions`)}),(!r?.privacy||r.privacy.showInFooter!==!1)&&t.push({href:`/privacy`,label:n(r?.privacy?.title,r?.privacy?.titleEn)||n(`حریم خصوصی`,`Privacy Policy`)}),(!r?.about||r.about.showInFooter!==!1)&&t.push({href:`/about`,label:n(r?.about?.title,r?.about?.titleEn)||n(`درباره ما`,`About Us`)}),Array.isArray(e?.nav_pages))for(let r of e.nav_pages)r.showInFooter&&t.push({href:`/page/${encodeURIComponent(r.slug)}`,label:n(r.title,r.titleEn)||r.title});return t}function I(e){return F(e).map(e=>`<li><a href="${e.href}">${e.label}</a></li>`).join(``)}var L={fa:`بهبار`,en:`Behbar`},R={telegram:`telegramFilled`,whatsapp:`whatsappFilled`,instagram:`instagramFilled`,linkedin:`linkedinFilled`,youtube:`youtubeFilled`,twitterX:`twitterXFilled`,facebook:`facebookFilled`,mail:`mailFilled`,globe:`globeFilled`},z={googlePlay:`Google Play`,appStore:`App Store`,bazaar:`کافه‌بازار`,custom:``};function B(t,n){return t.length?`
    <div class="footer-social" ${n}>
      ${t.map(t=>{let n=t.customIconUrl?`<img src="${t.customIconUrl}" alt="${t.label}" style="width:20px;height:20px;object-fit:contain;border-radius:4px;" />`:e[R[t.platform]??`globe`],r=t.platform===`mail`;return`<a href="${t.url}" ${r?``:`target="_blank" rel="noopener"`} aria-label="${t.label}"><span class="icon">${n}</span></a>`}).join(``)}
    </div>
  `:``}function V(t){return!t?.enabled||!t.links.length?``:`
      ...
      <div class="footer-app-links">
        ${t.links.map(t=>`
          <a class="footer-app-badge" href="${t.url}" target="_blank" rel="noopener">
            ...
            <span>${t.label||z[t.platform]}</span>
          </a>
        `).join(``)}
      </div>
    </div>
  `}function H(e){return!e?.enabled||!e.badges.length?``:`
    <div class="footer-extras-column">
  ...
  `}function G(e){if(e){let t=document.querySelector(`.site-footer .footer-nav ul`);t&&(t.innerHTML=I(e))}let t=document.getElementById(`footer-seo-box`),r=document.getElementById(`footer-seo-toggle`),i=r?.querySelector(`.footer-seo-toggle-label`);!t||!r||!i||r.addEventListener(`click`,()=>{let e=t.classList.toggle(`is-expanded`);r.setAttribute(`aria-expanded`,String(e)),i.textContent=e?n(`بستن`,`Close`):n(`ادامه مطلب`,`Read more`)})}function K(){return`
  ...
  `}function q(){let e=document.querySelectorAll(`.bottom-nav-item[href]`),t=location.pathname;for(let n of e){let e=(n.getAttribute(`href`)??`/`).split(`#`)[0]||`/`;if(e===`/`?t===`/`:t.startsWith(e)){n.classList.add(`is-active`);break}}document.getElementById(`bottom-nav-chat-btn`)?.addEventListener(`click`,()=>{document.getElementById(`chat-widget-toggle`)?.click()})}function ne(e){a(),e(),l(()=>e())}var J=null,Y=null;async function X(){return J||Y||(Y=fetch(`${o}/api/settings`).then(e=>e.json()).then(e=>(J=e?.settings??{},J)).catch(()=>({})),Y)}async function Z(){try{return(await(await fetch(`https://api.behbarapp.ir/api/magazine/articles`)).json().catch(()=>({})))?.articles??[]}catch{return[]}}async function re(e){try{let t=await fetch(`${o}/api/magazine/articles/${encodeURIComponent(e)}`);return t.ok?(await t.json().catch(()=>({})))?.article??null:null}catch{return null}}async function ie(e){try{let t=await fetch(`${o}/api/pages/${encodeURIComponent(e)}`);return t.ok?(await t.json().catch(()=>({})))?.page??null:null}catch{return null}}async function ae(){try{return(await(await fetch(`https://api.behbarapp.ir/api/testimonials`)).json().catch(()=>({})))?.testimonials??[]}catch{return[]}}async function oe(){try{return(await(await fetch(`https://api.behbarapp.ir/api/stories`)).json().catch(()=>({})))?.stories??[]}catch{return[]}}var se={primary:`--primary`,primaryDark:`--primary-dark`,secondary:`--secondary`,secondaryLight:`--secondary-light`,background:`--background`,surface:`--surface`,surfaceAlt:`--surface-alt`,text:`--text`,muted:`--muted`,border:`--border`,success:`--success`,successDark:`--success-dark`,successBg:`--success-bg`,warning:`--warning`,callGreen:`--call-green`,callGreenDark:`--call-green-dark`,accentPurple:`--accent-purple`,accentPurpleLight:`--accent-purple-light`,accentPurpleLightHover:`--accent-purple-light-hover`,gooseGreen:`--goose-green`,gooseGreenLight:`--goose-green-light`},Q=/^#[0-9a-fA-F]{6}$/;function ce(e){document.documentElement.classList.toggle(`quick-actions-fixed`,e?.quickActionsStyle===`fixed`);let t=localStorage.getItem(`behbar_quick_actions_side`),n=t===`left`||t===`right`?t:e?.quickActionsPosition===`left`?`left`:`right`;if(document.documentElement.classList.toggle(`quick-actions-left`,n===`left`),document.documentElement.classList.toggle(`quick-actions-right`,n===`right`),!e)return;let r=document.documentElement.style;for(let[t,n]of Object.entries(se)){let i=e[t];i&&Q.test(i)&&r.setProperty(n,i)}}function $(e,t=`name`){let n=document.head.querySelector(`meta[${t}="${e}"]`);return n||(n=document.createElement(`meta`),n.setAttribute(t,e),document.head.appendChild(n)),n}function le(e){if(e){if(e.googleSiteVerification&&$(`google-site-verification`).setAttribute(`content`,e.googleSiteVerification),e.googleAnalyticsId&&!document.getElementById(`ga4-loader`)){let t=document.createElement(`script`);t.id=`ga4-loader`,t.async=!0,t.src=`https://www.googletagmanager.com/gtag/js?id=${encodeURIComponent(e.googleAnalyticsId)}`,document.head.appendChild(t);let n=document.createElement(`script`);n.id=`ga4-inline`,n.textContent=`window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments);}gtag('js',new Date());gtag('config','${e.googleAnalyticsId}');`,document.head.appendChild(n)}e.defaultOgImage&&($(`og:image`,`property`).setAttribute(`content`,e.defaultOgImage),$(`twitter:image`).setAttribute(`content`,e.defaultOgImage))}}function ue(e,t){let n=t?.fa?.trim()||`بهبار`,r=e?.headline?.fa?.trim()||`حمل و جابه‌جایی، ساده‌تر از همیشه`,i=`${n} | ${r} - ${e?.subtitle?.fa?.trim()||`برای اثاث‌کشی یا حمل بار درخواست خود را ثبت کنید؛ در سریع‌ترین زمان با شما هماهنگ می‌کنیم.`}`;$(`description`).setAttribute(`content`,i),$(`og:description`,`property`).setAttribute(`content`,i),$(`og:title`,`property`).setAttribute(`content`,`${n} | ${r}`),$(`og:site_name`,`property`).setAttribute(`content`,n),$(`twitter:description`).setAttribute(`content`,i),$(`twitter:title`).setAttribute(`content`,`${n} | ${r}`),document.querySelectorAll(`script[type="application/ld+json"]`).forEach(e=>{try{let t=JSON.parse(e.textContent??``),r=!1;(t[`@type`]===`WebSite`||t[`@type`]===`LocalBusiness`||t[`@type`]===`Organization`)&&(t.description=i,t.name&&=n,r=!0),r&&(e.textContent=JSON.stringify(t))}catch{}})}function de(e){let t=e.metaTitle||e.title,n=e.metaDescription||e.excerpt,r=`${location.origin}/magazine/${e.slug}`;document.title=t,$(`description`).setAttribute(`content`,n),$(`og:title`,`property`).setAttribute(`content`,t),$(`og:description`,`property`).setAttribute(`content`,n),$(`og:url`,`property`).setAttribute(`content`,r),$(`twitter:title`).setAttribute(`content`,t);let i=document.head.querySelector(`link[rel="canonical"]`);i||(i=document.createElement(`link`),i.rel=`canonical`,document.head.appendChild(i)),i.href=r,e.coverImageUrl&&($(`og:image`,`property`).setAttribute(`content`,e.coverImageUrl),$(`twitter:image`).setAttribute(`content`,e.coverImageUrl));let a=document.createElement(`script`);a.type=`application/ld+json`,a.textContent=JSON.stringify({"@context":`https://schema.org`,"@type":`Article`,headline:t,description:n,...e.coverImageUrl?{image:e.coverImageUrl}:{},...e.publishedAt?{datePublished:e.publishedAt}:{},mainEntityOfPage:r}),document.head.appendChild(a)}function fe(e){let t=e.metaTitle||e.title,n=e.metaDescription||e.excerpt,r=`${location.origin}/p/${e.slug}`;document.title=t,$(`description`).setAttribute(`content`,n),$(`og:title`,`property`).setAttribute(`content`,t),$(`og:description`,`property`).setAttribute(`content`,n),$(`og:url`,`property`).setAttribute(`content`,r),$(`twitter:title`).setAttribute(`content`,t);let i=document.head.querySelector(`link[rel="canonical"]`);i||(i=document.createElement(`link`),i.rel=`canonical`,document.head.appendChild(i)),i.href=r,e.coverImageUrl&&($(`og:image`,`property`).setAttribute(`content`,e.coverImageUrl),$(`twitter:image`).setAttribute(`content`,e.coverImageUrl));let a=document.createElement(`script`);a.type=`application/ld+json`,a.textContent=JSON.stringify({"@context":`https://schema.org`,"@type":`WebPage`,headline:t,description:n,...e.coverImageUrl?{image:e.coverImageUrl}:{},mainEntityOfPage:r}),document.head.appendChild(a)}function pe(e){let t=e?.fa?.trim();if(!(!t||t===`بهبار`)&&(document.title.includes(`بهبار`)&&(document.title=document.title.replace(/بهبار/g,t)),document.querySelectorAll(`meta[property="og:site_name"], meta[property="og:title"], meta[name="twitter:title"]`).forEach(e=>{let n=e.getAttribute(`content`);n?.includes(`بهبار`)&&e.setAttribute(`content`,n.replace(/بهبار/g,t))}),document.querySelectorAll(`script[type="application/ld+json"]`).forEach(e=>{try{let n=JSON.parse(e.textContent??``),r=!1;n.name===`بهبار`&&(n.name=t,r=!0);let i=n.isPartOf;i?.name===`بهبار`&&(i.name=t,r=!0),r&&(e.textContent=JSON.stringify(n))}catch{}}),typeof document<`u`&&document.body)){let e=document.createTreeWalker(document.body,NodeFilter.SHOW_TEXT,{acceptNode(e){let t=e.parentElement;if(!t)return NodeFilter.FILTER_SKIP;let n=t.tagName;return n===`SCRIPT`||n===`STYLE`||n===`INPUT`||n===`TEXTAREA`||n===`CODE`?NodeFilter.FILTER_SKIP:e.nodeValue?.includes(`بهبار`)?NodeFilter.FILTER_ACCEPT:NodeFilter.FILTER_SKIP}}),n=[];for(;e.nextNode();)n.push(e.currentNode);for(let e of n)e.nodeValue=(e.nodeValue||``).replace(/بهبار/g,t)}}function me(e){if(!e)return;e.logoUrl&&document.querySelectorAll(`.header-logo img, .logo-mark`).forEach(t=>{t.src=e.logoUrl});let t=e.faviconUrl||e.logoUrl;t&&document.querySelectorAll(`link[rel="icon"], link[rel="apple-touch-icon"]`).forEach(e=>{e.href=t})}function he(e){!e||e===`both`||t()!==e&&i(e)}function ge(e){!e||e===`both`||document.querySelectorAll(`[data-lang-toggle]`).forEach(e=>{e.style.display=`none`})}export{O as C,M as S,T,q as _,de as a,W as b,le as c,Z as d,ie as f,ne as g,X as h,pe as i,ce as l,ae as m,ge as n,ue as o,oe as p,me as r,fe as s,he as t,re as u,K as v,E as w,P as x,G as y};