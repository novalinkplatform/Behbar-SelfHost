import{B as e,C as t,F as n,I as r,L as i,P as a,R as o,S as s,V as c,_ as l,a as u,b as d,c as f,g as p,h as m,i as h,l as g,n as _,r as v,t as y,v as b,x,y as S}from"./appReady-DNSS-HAw.js";import{n as C}from"./services-BoeLN-Ce.js";var w=[{id:`driver`,label:`راننده`,labelEn:`Driver`},{id:`worker`,label:`کارگر`,labelEn:`Laborer`},{id:`other`,label:`سایر`,labelEn:`Other`}];function T(t=C){return`
    <article class="orders-page careers-page">
      <div class="container orders-container">
        ...
        <h1 class="article-title">${e(`فرصت‌های شغلی بهبار`,`Careers at Behbar`)}</h1>
        <p class="article-excerpt">${e(`به تیم بهبار بپیوندید؛ به نیروی راننده، کارگر و سایر همکاران نیاز داریم. فرم زیر را پر کنید تا همکاران ما با شما تماس بگیرند.`,`Join the Behbar team — we’re hiring drivers, laborers, and other roles. Fill out the form below and our team will get in touch.`)}</p>
              ...
              <div class="form-field">
                ...
                <div class="select-wrapper">
                  <select id="careers-position">
                    ${w.map(t=>`<option value="${t.id}">${e(t.label,t.labelEn)}</option>`).join(``)}
                  </select>
  ...
  `}function E(){let t=document.getElementById(`careers-form`),n=document.getElementById(`careers-form-card`),r=document.getElementById(`careers-success`),i=document.getElementById(`careers-form-error`),a=document.getElementById(`careers-position`),o=document.getElementById(`careers-custom-position-field`),s=document.getElementById(`careers-vehicle-field`),c=document.getElementById(`careers-has-vehicle`),l=document.getElementById(`careers-vehicle-type-field`),u=document.getElementById(`careers-submit-btn`);if(!t||!n||!r||!i||!a||!o||!s||!c||!l||!u)return;function d(){o.hidden=a.value!==`other`,s.hidden=a.value!==`driver`,l.hidden=c.value!==`yes`}a.addEventListener(`change`,d),c.addEventListener(`change`,d),d(),t.addEventListener(`submit`,async t=>{t.preventDefault(),i.hidden=!0;let o=document.getElementById(`careers-name`).value.trim(),s=document.getElementById(`careers-phone`).value.trim(),l=a.value,d=document.getElementById(`careers-custom-position`).value.trim(),f=document.getElementById(`careers-city`).value.trim(),p=document.getElementById(`careers-message`).value.trim(),m=l===`driver`?c.value===`yes`:void 0,h=m?document.getElementById(`careers-vehicle-type`).value:void 0;if(!o||!s){i.hidden=!1,i.textContent=e(`نام و شماره موبایل الزامی است.`,`Full name and mobile number are required.`);return}if(l===`other`&&!d){i.hidden=!1,i.textContent=e(`عنوان شغلی مورد نظر را وارد کنید.`,`Please enter the desired position title.`);return}u.disabled=!0;try{let t=await fetch(`/api/job-applications`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({fullName:o,phone:s,position:l,positionLabel:d,city:f||void 0,message:p||void 0,hasVehicle:m,vehicleType:h})}),i=await t.json().catch(()=>({}));if(!t.ok)throw Error(typeof i?.error==`string`?i.error:e(`ارسال درخواست ناموفق بود.`,`Failed to submit the application.`));n.hidden=!0,r.hidden=!1}catch(t){i.hidden=!1,i.textContent=t instanceof Error?t.message:e(`ارسال درخواست ناموفق بود.`,`Failed to submit the application.`)}finally{u.disabled=!1}})}function D(r){let a=document.querySelector(`#app`);if(!a)return;let o=r.vehicle_types?.length?r.vehicle_types:C;a.innerHTML=`
