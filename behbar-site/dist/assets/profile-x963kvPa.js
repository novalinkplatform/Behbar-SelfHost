import{S as e,b as t,c as n,f as r,l as i,n as a,t as o}from"./appReady-DVpveje2.js";import{C as s,S as c,T as l,_ as u,b as d,c as f,g as p,h as m,i as h,l as g,n as ee,r as te,t as ne,v as _,w as re,x as ie,y as ae}from"./languageMode-MHBiRO0M.js";import{a as oe,c as se,d as ce,i as le,n as ue,o as v,r as de,s as fe,t as pe}from"./customerAuth-B9gm3CQT.js";import{n as me,t as he}from"./addresses-C0C7lU78.js";function y(e,n=6){return`
  ...
  `)}function be(e){if(!e.length)return t(`نامشخص`,`Unknown`);let r=e.reduce((e,t)=>new Date(t.createdAt)<new Date(e.createdAt)?t:e);return n(i(new Date(r.createdAt)))}async function S(){a(),x=await m(),ne(x.language_mode),g(x.theme),f(x.seo),ye(),o(),te(x.branding),h(x.site_name),ee(x.language_mode),ie(x),ae(x),u(),s(),re(x);let n=document.getElementById(`profile-auth-section`),i=document.getElementById(`profile-page-content`),c=document.getElementById(`profile-info-card`),l=document.getElementById(`profile-addresses-block`),d=document.getElementById(`profile-origin-addresses`),p=document.getElementById(`profile-origin-address-list`),_=document.getElementById(`profile-destination-addresses`),y=document.getElementById(`profile-destination-address-list`),b=document.getElementById(`profile-logout-btn`),S=document.getElementById(`profile-phone-form`),C=document.getElementById(`profile-phone-input`),w=document.getElementById(`profile-phone-submit-btn`),T=document.getElementById(`profile-phone-error`),E=document.getElementById(`profile-otp-form`),D=document.getElementById(`profile-otp-phone-display`),O=document.getElementById(`profile-change-phone-btn`),k=document.getElementById(`profile-otp-submit-btn`),A=document.getElementById(`profile-otp-error`),j=document.getElementById(`profile-details-form`),M=Array.from(document.querySelectorAll(`.profile-gender-btn`)),N=document.getElementById(`profile-gender-input`),P=document.getElementById(`profile-fullname-input`),F=document.getElementById(`profile-fullname-label`),I=document.getElementById(`profile-company-field`),L=document.getElementById(`profile-company-input`),R=document.getElementById(`profile-company-label`),z=document.getElementById(`profile-details-submit-btn`),B=document.getElementById(`profile-details-error`),V=document.getElementById(`profile-add-address-btn`),H=document.getElementById(`profile-new-address-form`),U=document.getElementById(`profile-cancel-address-btn`),W=document.getElementById(`profile-address-title-input`),xe=document.getElementById(`profile-address-city-input`),Se=document.getElementById(`profile-address-text-input`),Ce=document.getElementById(`profile-address-floor-input`),we=document.getElementById(`profile-address-unit-input`),Te=document.getElementById(`profile-address-elevator-input`),G=document.getElementById(`profile-address-form-error`),K=document.getElementById(`profile-addresses-grid`),Ee=Array.from(document.querySelectorAll(`.profile-chip`));if(!n||!i||!c||!l||!d||!p||!_||!y||!b||!S||!C||!w||!T||!E||!D||!O||!k||!A||!j||!N||!P||!z||!B)return;let q=``,J=null;async function Y(){if(!K)return;let n=await le();if(!n.length){K.innerHTML=`
      ...
      `;return}K.innerHTML=n.map(n=>{let i=[];return n.city&&i.push(`<span class="profile-address-tag">${n.city}</span>`),n.floor&&i.push(`<span class="profile-address-tag">${t(`طبقه`,`Floor`)} ${r(n.floor)}</span>`),n.unit&&i.push(`<span class="profile-address-tag">${t(`واحد`,`Unit`)} ${r(n.unit)}</span>`),n.hasElevator&&i.push(`<span class="profile-address-tag">${t(`دارای آسانسور`,`With elevator`)}</span>`),`
          <div class="profile-address-card" data-address-id="${n.id}">
            ...
            ${i.length?`<div class="profile-address-tags">${i.join(``)}</div>`:``}
          </div>
        `}).join(``),Array.from(K.querySelectorAll(`.profile-address-del-btn`)).forEach(e=>{e.addEventListener(`click`,()=>{let n=Number(e.dataset.deleteId);n&&(e.disabled=!0,ue(n).then(()=>Y()).catch(n=>{alert(n instanceof Error?n.message:t(`حذف ناموفق بود.`,`Failed to delete`)),e.disabled=!1}))})})}function De(n,i){let a=be(i),o=e.user,s=``,u=`profile-badge-male`;n.gender===`female`?(o=e.female,s=t(`خانم`,`Female`),u=`profile-badge-female`):n.gender===`male`?(o=e.male,s=t(`آقا`,`Male`),u=`profile-badge-male`):n.gender===`company`?(o=e.building,s=t(`شرکتی`,`Company`),u=`profile-badge-company`):n.gender===`organization`&&(o=e.organization,s=t(`اداری و سازمانی`,`Organization`),u=`profile-badge-org`);let f=n.companyName?`
        <div class="orders-profile-row">
          ...
          ${n.gender?`<span class="profile-gender-badge ${u}">${s}</span>`:``}
        </div>
      `:``,m=!n.companyName&&s?`<span class="profile-gender-badge ${u}">${s}</span>`:``;c.innerHTML=`
      ${f}
      <div class="orders-profile-row">
      ...
      <div class="orders-profile-row">
        ...
        <span dir="ltr">${r(n.phone)}</span>
      </div>
      <div class="orders-profile-row">
        <span class="icon">${e.calendar}</span>
        <span>${t(`عضویت از`,`Member since`)} ${a}</span>
      </div>
    `,Y();let{origins:h,destinations:g}=he(i);h.length?(d.hidden=!1,p.innerHTML=h.map(t=>`<li class="saved-address-item"><span class="icon">${e.pin}</span><span>${me(t)}</span></li>`).join(``)):d.hidden=!0,g.length?(_.hidden=!1,y.innerHTML=g.map(t=>`<li class="saved-address-item"><span class="icon">${e.flag}</span><span>${me(t)}</span></li>`).join(``)):_.hidden=!0,l.hidden=!h.length&&!g.length}function X(e){n.hidden=!0,i.hidden=!1,ce(e.phone).then(t=>De(e,t)).catch(()=>De(e,[]))}function Z(){n.hidden=!1,i.hidden=!0,S.hidden=!1,E.hidden=!0,j.hidden=!0,T.hidden=!0,C.focus()}function Oe(e){q=e,n.hidden=!1,i.hidden=!0,S.hidden=!0,E.hidden=!1,j.hidden=!0,A.hidden=!0,D.textContent=r(e),J?.stopWebOtp(),J=ge(`profile-otp`,{length:5,onComplete:e=>{$(e)},onResend:()=>{Ae()}}),J?.focusFirst()}function ke(e){n.hidden=!1,i.hidden=!0,S.hidden=!0,E.hidden=!0,j.hidden=!1,B.hidden=!0,e.fullName&&(P.value=e.fullName),e.companyName&&L&&(L.value=e.companyName),e.gender?Q(e.gender):Q(``),e.gender===`company`||e.gender===`organization`?L?.focus():P.focus()}function Q(e){N.value=e,M.forEach(t=>{let n=t.dataset.gender===e;t.classList.toggle(`is-active`,n),t.setAttribute(`aria-checked`,String(n))});let n=e===`company`||e===`organization`;I&&(I.hidden=!n,n?(R&&(R.textContent=e===`company`?t(`نام شرکت یا مجموعه تجاری`,`Company name`):t(`نام اداره یا سازمان`,`Organization name`)),L&&(L.placeholder=e===`company`?t(`نام شرکت، فروشگاه یا برند تجاری`,`Enter company name`):t(`نام اداره، سازمان یا نهاد دولتی/عمومی`,`Enter organization name`),L.required=!0)):L&&(L.required=!1,L.value=``)),F&&P&&(n?(F.textContent=t(`نام و نام خانوادگی رابط / نماینده`,`Representative full name`),P.placeholder=t(`نام و نام خانوادگی شخص رابط یا مسئول هماهنگی`,`Representative name`)):(F.textContent=t(`نام و نام خانوادگی`,`Full name`),P.placeholder=t(`نام و نام خانوادگی خود را وارد کنید`,`Enter your full name`)))}M.forEach(e=>{e.addEventListener(`click`,()=>{Q(e.dataset.gender??``),B.hidden=!0})}),V&&H&&V.addEventListener(`click`,()=>{H.hidden=!H.hidden,!H.hidden&&W&&W.focus()}),U&&H&&U.addEventListener(`click`,()=>{H.hidden=!0,H.reset(),G&&(G.hidden=!0)}),Ee.forEach(e=>{e.addEventListener(`click`,()=>{W&&(W.value=e.dataset.chip??e.textContent??``,W.focus())})}),H&&H.addEventListener(`submit`,e=>{e.preventDefault(),G&&(G.hidden=!0);let n=W?.value.trim()??``,r=xe?.value.trim()??`تهران`,i=Se?.value.trim()??``,a=Ce?.value.trim()||void 0,o=we?.value.trim()||void 0,s=Te?.checked??!1;if(!n){G&&(G.hidden=!1,G.textContent=t(`لطفاً عنوان آدرس را وارد کنید.`,`Please enter address title.`));return}if(!i||i.length<5){G&&(G.hidden=!1,G.textContent=t(`لطفاً نشانی کامل را وارد کنید.`,`Please enter full address.`));return}let c=document.getElementById(`profile-save-address-btn`);c&&(c.disabled=!0,c.textContent=t(`در حال ذخیره...`,`Saving...`)),pe({title:n,city:r,address:i,floor:a,unit:o,hasElevator:s}).then(()=>{H.reset(),H.hidden=!0,Y()}).catch(e=>{G&&(G.hidden=!1,G.textContent=e instanceof Error?e.message:t(`خطایی در ثبت آدرس پیش آمد.`,`Failed to add address.`))}).finally(()=>{c&&(c.disabled=!1,c.textContent=t(`ذخیره آدرس`,`Save Address`))})}),O.addEventListener(`click`,()=>{J?.stopWebOtp(),Z()}),S.addEventListener(`submit`,e=>{e.preventDefault();let n=C.value,r=ve(n);if(T.hidden=!0,!_e.test(r)){T.hidden=!1,T.textContent=t(`شماره موبایل معتبر ۱۱ رقمی وارد کنید (مثال: ۰۹۱۲۳۴۵۶۷۸۹).`,`Enter a valid 11-digit mobile number.`);return}w.disabled=!0,w.textContent=t(`در حال ارسال کد...`,`Sending code...`),v(r).then(()=>{Oe(r)}).catch(e=>{T.hidden=!1,T.textContent=e instanceof Error?e.message:t(`خطایی در ارسال کد پیش آمد.`,`Failed to send code.`)}).finally(()=>{w.disabled=!1,w.textContent=t(`دریافت کد تأیید`,`Send verification code`)})});function Ae(){return A.hidden=!0,v(q).catch(e=>{A.hidden=!1,A.textContent=e instanceof Error?e.message:t(`ارسال مجدد کد با خطا مواجه شد.`,`Failed to resend code.`)})}async function $(e){A.hidden=!0,k.disabled=!0,k.textContent=t(`در حال بررسی...`,`Verifying...`);try{let t=await se(q,e);J?.stopWebOtp(),t.needsProfile?ke(t.customer):X(t.customer)}catch(e){A.hidden=!1,A.textContent=e instanceof Error?e.message:t(`کد واردشده معتبر نیست.`,`Invalid code.`),J?.reset()}finally{k.disabled=!1,k.textContent=t(`تأیید و ادامه`,`Verify and continue`)}}E.addEventListener(`submit`,e=>{e.preventDefault();let n=J?.getCode()??``;if(n.length!==5){A.hidden=!1,A.textContent=t(`لطفاً کد ۵ رقمی را کامل وارد کنید.`,`Please enter the full 5-digit code.`);return}$(n)}),j.addEventListener(`submit`,e=>{e.preventDefault(),B.hidden=!0;let n=N.value,r=P.value.trim(),i=L?.value.trim()||void 0;if(!n||![`female`,`male`,`company`,`organization`].includes(n)){B.hidden=!1,B.textContent=t(`لطفاً نوع حساب کاربری را انتخاب کنید.`,`Please select account type.`);return}if((n===`company`||n===`organization`)&&!i){B.hidden=!1,B.textContent=n===`company`?t(`لطفاً نام شرکت را وارد کنید.`,`Please enter company name.`):t(`لطفاً نام اداره یا سازمان را وارد کنید.`,`Please enter organization name.`);return}if(!r||r.length<2){B.hidden=!1,B.textContent=t(`لطفاً نام و نام خانوادگی را وارد کنید.`,`Please enter full name.`);return}z.disabled=!0,z.textContent=t(`در حال ثبت...`,`Saving...`),fe(r,n,i).then(e=>{X(e)}).catch(e=>{B.hidden=!1,B.textContent=e instanceof Error?e.message:t(`خطایی در ثبت اطلاعات رخ داد.`,`Failed to save profile.`)}).finally(()=>{z.disabled=!1,z.textContent=t(`تکمیل و ورود به حساب`,`Complete & Enter`)})}),b.addEventListener(`click`,()=>{oe().finally(()=>{Z(),S.reset(),E.reset(),j.reset(),Q(``)})}),de().then(e=>{e?!e.fullName||!e.gender?ke(e):X(e):Z()}).catch(()=>Z())}p(()=>void S());