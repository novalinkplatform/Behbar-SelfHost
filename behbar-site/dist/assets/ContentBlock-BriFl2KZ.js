import{C as e,h as t,p as n,x as r}from"./appReady-giJ54-6J.js";function i(e){return e.replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`)}function a(e,t,n){return i(r(e.heading||t,e.headingEn||e.heading||n))}function o(e){let t=r(e.subheading??``,e.subheadingEn??e.subheading??``);return t?`<p class="section-description">${i(t)}</p>`:``}function s(e){return i(r(e.title??``,e.titleEn??e.title??``))}function c(e){return i(r(e.text??``,e.textEn??e.text??``))}function l(e,t){if(e.videoUrl){let n=/\.(mp4|webm|ogg)(\?|$)/i.test(e.videoUrl),r=i(e.videoUrl);return n?`<video class="${t}" src="${r}" controls preload="metadata"></video>`:`<iframe class="${t}" src="${r}" loading="lazy" allowfullscreen></iframe>`}return e.imageUrl?`<img class="${t}" src="${i(e.imageUrl)}" alt="" loading="lazy" />`:``}function u(e,t){return`
  ...
  `}function b(e){let t=l(e,`content-slide-media`),n=s(e),a=c(e);return`
    <li class="content-slide">
      ...
      ${n||a||e.linkUrl?`
        <div class="content-slide-body">
          ${n?`<h3>${n}</h3>`:``}
          ${a?`<p>${a}</p>`:``}
          ${e.linkUrl?`<a class="content-slide-link" href="${i(e.linkUrl)}">${r(`بیشتر بدانید`,`Learn more`)}</a>`:``}
        </div>
      `:``}
    </li>
  `}function x(e){return`
  ...
  `}function T(e){return e.trim().slice(0,1)||`؟`}function E(e){return Array.from({length:5},(t,n)=>n<e?`★`:`☆`).join(``)}function D(e){let t=i(r(e.customerName,e.customerNameEn||e.customerName)),n=i(r(e.text,e.textEn||e.text));return`
    <li class="testimonial-card">
      <p class="testimonial-stars" aria-hidden="true">${E(e.rating)}</p>
      <p class="testimonial-text">${n}</p>
      <div class="testimonial-author">
        <span class="testimonial-avatar">
          ${e.avatarUrl?`<img src="${i(e.avatarUrl)}" alt="" loading="lazy" />`:`<span>${T(t)}</span>`}
        </span>
        <span class="testimonial-name">${t}</span>
      </div>
    </li>
      ...
      <form class="testimonial-submit-form" id="testimonial-submit-form" hidden>
        <div class="testimonial-submit-fields" id="testimonial-submit-fields">
          <div class="form-field">
            <label for="testimonial-submit-name">${r(`نام شما`,`Your name`)}</label>
            <input type="text" id="testimonial-submit-name" maxlength="80" required />
          </div>
          <div class="form-field">
            <label for="testimonial-submit-rating">${r(`امتیاز`,`Rating`)}</label>
          ...
          <div class="form-field">
            <label for="testimonial-submit-text">${r(`متن نظر`,`Your review`)}</label>
            <textarea id="testimonial-submit-text" maxlength="1000" rows="3" required></textarea>
          </div>
          <button type="submit" class="btn btn-primary" id="testimonial-submit-btn">${r(`ارسال`,`Submit`)}</button>
        </div>
        <p class="testimonial-submit-message" id="testimonial-submit-message" hidden></p>
      </form>
    </div>
  `}function A(e){let t=r(e.body??``,e.bodyEn??e.body??``);return t?`<div class="rich-text-body">${t.split(`
