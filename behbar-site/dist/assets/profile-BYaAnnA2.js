import{B as e,C as t,D as n,E as r,L as i,N as a,O as o,P as s,S as c,T as l,V as u,_ as d,a as f,b as p,g as m,i as h,l as g,n as ee,r as te,t as ne,u as re,v as ie,w as ae,y as _}from"./appReady-BWGY9_tm.js";import{a as oe,c as se,d as ce,i as le,n as ue,o as v,r as de,s as fe,t as pe}from"./customerAuth-NoQE0YwS.js";import{n as me,t as he}from"./addresses-sU6anDog.js";function y(t,n=6){return`
  ...
  `)}function be(t){if(!t.length)return e(`نامشخص`,`Unknown`);let n=t.reduce((e,t)=>new Date(t.createdAt)<new Date(e.createdAt)?t:e);return a(s(new Date(n.createdAt)))}async function S(){p(),x=await m(),ee(x.language_mode),re(x.theme),g(x.seo),ye(),ne(),h(x.branding),f(x.site_name),te(x.language_mode),ae(x),c(x),ie(),r(),n(x);let t=document.getElementById(`profile-auth-section`),a=document.getElementById(`profile-page-content`),o=document.getElementById(`profile-info-card`),s=document.getElementById(`profile-addresses-block`),l=document.getElementById(`profile-origin-addresses`),d=document.getElementById(`profile-origin-address-list`),_=document.getElementById(`profile-destination-addresses`),y=document.getElementById(`profile-destination-address-list`),b=document.getElementById(`profile-logout-btn`),S=document.getElementById(`profile-phone-form`),C=document.getElementById(`profile-phone-input`),w=document.getElementById(`profile-phone-submit-btn`),T=document.getElementById(`profile-phone-error`),E=document.getElementById(`profile-otp-form`),D=document.getElementById(`profile-otp-phone-display`),O=document.getElementById(`profile-change-phone-btn`),k=document.getElementById(`profile-otp-submit-btn`),A=document.getElementById(`profile-otp-error`),j=document.getElementById(`profile-details-form`),M=Array.from(document.querySelectorAll(`.profile-gender-btn`)),N=document.getElementById(`profile-gender-input`),P=document.getElementById(`profile-fullname-input`),F=document.getElementById(`profile-fullname-label`),I=document.getElementById(`profile-company-field`),L=document.getElementById(`profile-company-input`),R=document.getElementById(`profile-company-label`),z=document.getElementById(`profile-details-submit-btn`),B=document.getElementById(`profile-details-error`),V=document.getElementById(`profile-add-address-btn`),H=document.getElementById(`profile-new-address-form`),U=document.getElementById(`profile-cancel-address-btn`),W=document.getElementById(`profile-address-title-input`),xe=document.getElementById(`profile-address-city-input`),Se=document.getElementById(`profile-address-text-input`),Ce=document.getElementById(`profile-address-floor-input`),we=document.getElementById(`profile-address-unit-input`),Te=document.getElementById(`profile-address-elevator-input`),G=document.getElementById(`profile-address-form-error`),K=document.getElementById(`profile-addresses-grid`),Ee=Array.from(document.querySelectorAll(`.profile-chip`));if(!t||!a||!o||!s||!l||!d||!_||!y||!b||!S||!C||!w||!T||!E||!D||!O||!k||!A||!j||!N||!P||!z||!B)return;let q=``,J=null;async function Y(){if(!K)return;let t=await le();if(!t.length){K.innerHTML=`
      ...
      `;return}K.innerHTML=t.map(t=>{let n=[];return t.city&&n.push(`<span class="profile-address-tag">${t.city}</span>`),t.floor&&n.push(`<span class="profile-address-tag">${e(`طبقه`,`Floor`)} ${i(t.floor)}</span>`),t.unit&&n.push(`<span class="profile-address-tag">${e(`واحد`,`Unit`)} ${i(t.unit)}</span>`),t.hasElevator&&n.push(`<span class="profile-address-tag">${e(`دارای آسانسور`,`With elevator`)}</span>`),`
        ...
        `}).join(``),Array.from(K.querySelectorAll(`.profile-address-del-btn`)).forEach(t=>{t.addEventListener(`click`,()=>{let n=Number(t.dataset.deleteId);n&&(t.disabled=!0,ue(n).then(()=>Y()).catch(n=>{alert(n instanceof Error?n.message:e(`حذف ناموفق بود.`,`Failed to delete`)),t.disabled=!1}))})})}function De(t,n){let r=be(n),a=u.user,c=``,f=`profile-badge-male`;t.gender===`female`?(a=u.female,c=e(`خانم`,`Female`),f=`profile-badge-female`):t.gender===`male`?(a=u.male,c=e(`آقا`,`Male`),f=`profile-badge-male`):t.gender===`company`?(a=u.building,c=e(`شرکتی`,`Company`),f=`profile-badge-company`):t.gender===`organization`&&(a=u.organization,c=e(`اداری و سازمانی`,`Organization`),f=`profile-badge-org`);let p=t.companyName?`
        <div class="orders-profile-row">
          ...
          ${t.gender?`<span class="profile-gender-badge ${f}">${c}</span>`:``}
        </div>
      `:``,m=!t.companyName&&c?`<span class="profile-gender-badge ${f}">${c}</span>`:``;o.innerHTML=`
      ${p}
      <div class="orders-profile-row">
      ...
      <div class="orders-profile-row">
        ...
        <span dir="ltr">${i(t.phone)}</span>
      </div>
      <div class="orders-profile-row">
        <span class="icon">${u.calendar}</span>
        <span>${e(`عضویت از`,`Member since`)} ${r}</span>
      </div>
    `,Y();let{origins:h,destinations:g}=he(n);h.length?(l.hidden=!1,d.innerHTML=h.map(e=>`<li class="saved-address-item"><span class="icon">${u.pin}</span><span>${me(e)}</span></li>`).join(``)):l.hidden=!0,g.length?(_.hidden=!1,y.innerHTML=g.map(e=>`<li class="saved-address-item"><span class="icon">${u.flag}</span><span>${me(e)}</span></li>`).join(``)):_.hidden=!0,s.hidden=!h.length&&!g.length}function X(e){t.hidden=!0,a.hidden=!1,ce(e.phone).then(t=>De(e,t)).catch(()=>De(e,[]))}function Z(){t.hidden=!1,a.hidden=!0,S.hidden=!1,E.hidden=!0,j.hidden=!0,T.hidden=!0,C.focus()}function Oe(e){q=e,t.hidden=!1,a.hidden=!0,S.hidden=!0,E.hidden=!1,j.hidden=!0,A.hidden=!0,D.textContent=i(e),J?.stopWebOtp(),J=ge(`profile-otp`,{length:5,onComplete:e=>{$(e)},onResend:()=>{Ae()}}),J?.focusFirst()}function ke(e){t.hidden=!1,a.hidden=!0,S.hidden=!0,E.hidden=!0,j.hidden=!1,B.hidden=!0,e.fullName&&(P.value=e.fullName),e.companyName&&L&&(L.value=e.companyName),e.gender?Q(e.gender):Q(``),e.gender===`company`||e.gender===`organization`?L?.focus():P.focus()}function Q(t){N.value=t,M.forEach(e=>{let n=e.dataset.gender===t;e.classList.toggle(`is-active`,n),e.setAttribute(`aria-checked`,String(n))});let n=t===`company`||t===`organization`;I&&(I.hidden=!n,n?(R&&(R.textContent=t===`company`?e(`نام شرکت یا مجموعه تجاری`,`Company name`):e(`نام اداره یا سازمان`,`Organization name`)),L&&(L.placeholder=t===`company`?e(`نام شرکت، فروشگاه یا برند تجاری`,`Enter company name`):e(`نام اداره، سازمان یا نهاد دولتی/عمومی`,`Enter organization name`),L.required=!0)):L&&(L.required=!1,L.value=``)),F&&P&&(n?(F.textContent=e(`نام و نام خانوادگی رابط / نماینده`,`Representative full name`),P.placeholder=e(`نام و نام خانوادگی شخص رابط یا مسئول هماهنگی`,`Representative name`)):(F.textContent=e(`نام و نام خانوادگی`,`Full name`),P.placeholder=e(`نام و نام خانوادگی خود را وارد کنید`,`Enter your full name`)))}M.forEach(e=>{e.addEventListener(`click`,()=>{Q(e.dataset.gender??``),B.hidden=!0})}),V&&H&&V.addEventListener(`click`,()=>{H.hidden=!H.hidden,!H.hidden&&W&&W.focus()}),U&&H&&U.addEventListener(`click`,()=>{H.hidden=!0,H.reset(),G&&(G.hidden=!0)}),Ee.forEach(e=>{e.addEventListener(`click`,()=>{W&&(W.value=e.dataset.chip??e.textContent??``,W.focus())})}),H&&H.addEventListener(`submit`,t=>{t.preventDefault(),G&&(G.hidden=!0);let n=W?.value.trim()??``,r=xe?.value.trim()??`تهران`,i=Se?.value.trim()??``,a=Ce?.value.trim()||void 0,o=we?.value.trim()||void 0,s=Te?.checked??!1;if(!n){G&&(G.hidden=!1,G.textContent=e(`لطفاً عنوان آدرس را وارد کنید.`,`Please enter address title.`));return}if(!i||i.length<5){G&&(G.hidden=!1,G.textContent=e(`لطفاً نشانی کامل را وارد کنید.`,`Please enter full address.`));return}let c=document.getElementById(`profile-save-address-btn`);c&&(c.disabled=!0,c.textContent=e(`در حال ذخیره...`,`Saving...`)),pe({title:n,city:r,address:i,floor:a,unit:o,hasElevator:s}).then(()=>{H.reset(),H.hidden=!0,Y()}).catch(t=>{G&&(G.hidden=!1,G.textContent=t instanceof Error?t.message:e(`خطایی در ثبت آدرس پیش آمد.`,`Failed to add address.`))}).finally(()=>{c&&(c.disabled=!1,c.textContent=e(`ذخیره آدرس`,`Save Address`))})}),O.addEventListener(`click`,()=>{J?.stopWebOtp(),Z()}),S.addEventListener(`submit`,t=>{t.preventDefault();let n=C.value,r=ve(n);if(T.hidden=!0,!_e.test(r)){T.hidden=!1,T.textContent=e(`شماره موبایل معتبر ۱۱ رقمی وارد کنید (مثال: ۰۹۱۲۳۴۵۶۷۸۹).`,`Enter a valid 11-digit mobile number.`);return}w.disabled=!0,w.textContent=e(`در حال ارسال کد...`,`Sending code...`),v(r).then(()=>{Oe(r)}).catch(t=>{T.hidden=!1,T.textContent=t instanceof Error?t.message:e(`خطایی در ارسال کد پیش آمد.`,`Failed to send code.`)}).finally(()=>{w.disabled=!1,w.textContent=e(`دریافت کد تأیید`,`Send verification code`)})});function Ae(){return A.hidden=!0,v(q).catch(t=>{A.hidden=!1,A.textContent=t instanceof Error?t.message:e(`ارسال مجدد کد با خطا مواجه شد.`,`Failed to resend code.`)})}async function $(t){A.hidden=!0,k.disabled=!0,k.textContent=e(`در حال بررسی...`,`Verifying...`);try{let e=await se(q,t);J?.stopWebOtp(),e.needsProfile?ke(e.customer):X(e.customer)}catch(t){A.hidden=!1,A.textContent=t instanceof Error?t.message:e(`کد واردشده معتبر نیست.`,`Invalid code.`),J?.reset()}finally{k.disabled=!1,k.textContent=e(`تأیید و ادامه`,`Verify and continue`)}}E.addEventListener(`submit`,t=>{t.preventDefault();let n=J?.getCode()??``;if(n.length!==5){A.hidden=!1,A.textContent=e(`لطفاً کد ۵ رقمی را کامل وارد کنید.`,`Please enter the full 5-digit code.`);return}$(n)}),j.addEventListener(`submit`,t=>{t.preventDefault(),B.hidden=!0;let n=N.value,r=P.value.trim(),i=L?.value.trim()||void 0;if(!n||![`female`,`male`,`company`,`organization`].includes(n)){B.hidden=!1,B.textContent=e(`لطفاً نوع حساب کاربری را انتخاب کنید.`,`Please select account type.`);return}if((n===`company`||n===`organization`)&&!i){B.hidden=!1,B.textContent=n===`company`?e(`لطفاً نام شرکت را وارد کنید.`,`Please enter company name.`):e(`لطفاً نام اداره یا سازمان را وارد کنید.`,`Please enter organization name.`);return}if(!r||r.length<2){B.hidden=!1,B.textContent=e(`لطفاً نام و نام خانوادگی را وارد کنید.`,`Please enter full name.`);return}z.disabled=!0,z.textContent=e(`در حال ثبت...`,`Saving...`),fe(r,n,i).then(e=>{X(e)}).catch(t=>{B.hidden=!1,B.textContent=t instanceof Error?t.message:e(`خطایی در ثبت اطلاعات رخ داد.`,`Failed to save profile.`)}).finally(()=>{z.disabled=!1,z.textContent=e(`تکمیل و ورود به حساب`,`Complete & Enter`)})}),b.addEventListener(`click`,()=>{oe().finally(()=>{Z(),S.reset(),E.reset(),j.reset(),Q(``)})}),de().then(e=>{e?!e.fullName||!e.gender?ke(e):X(e):Z()}).catch(()=>Z())}d(()=>void S());