      ...
      <div class="container article-container">
        ...
        <div class="article-card">
          ...
          <div class="article-cta">
            <h3>${pick('آماده جابه‌جایی هستید؟', 'Ready to move?')}</h3>
            <p>${pick(
              'مبدأ و مقصد را روی نقشه مشخص کنید و در چند دقیقه یک برآورد هزینه شفاف دریافت کنید.',
              'Pinpoint the origin and destination on the map and get a transparent cost estimate in minutes.',
            )}</p>
            <a class="btn btn-primary" href="/#request">
              <span class="icon">${icons.plusCircle}</span>
              ${pick('ثبت درخواست در بهبار', 'Submit a request on Behbar')}
            </a>
          </div>
        </div>
      </div>
